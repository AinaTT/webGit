 <!DOCTYPE html>
 <html lang="cn">

 <head>
     <meta charset="utf-8">
     <meta name="theme-color" content="#000"/>
     <meta name="viewport" content="width=device-width"/>

     <link rel="icon" type="image/png" href="https://www.topciment.com/favicon.png"/>
     <meta name="Language" content="CN"/>


     <?php include("../assets/includes/root-styles.php"); ?>

     <link rel="preload" href="../assets/css/fichas-tecnicas.css" as="style"/>
     <link rel="stylesheet" href="../assets/css/fichas-tecnicas.css"/>

     <title>Fichas técnicas | Topciment</title>
     <meta name="description" content="Descarga nuestro catálogo y nuestras fichas técnicas para eligir la solucion que mejor se ajuste a tus necesidades."/>

     <meta property="og:locale" content="zh_CN"/>
     <meta property="og:type" content="website"/>
     <meta property="og:title" content="Fichas técnicas | Topciment"/>
     <meta property="og:description" content="Descarga nuestro catálogo y nuestras fichas técnicas para eligir la solucion que mejor se ajuste a tus necesidades."/>
     <meta property="og:url" content="https://www.topciment.com/cn/jishucanshu"/>
     <meta property="og:site_name" content="Topciment"/>
     <meta name="twitter:card" content="summary"/>
     <meta name="twitter:description" content="Descarga nuestro catálogo y nuestras fichas técnicas para eligir la solucion que mejor se ajuste a tus necesidades."/>
     <meta name="twitter:title" content="Fichas técnicas | Topciment"/>
     <meta name="twitter:site" content="@topciment"/>
     <meta name="twitter:creator" content="@topciment"/>
     <meta property="article:publisher" content="https://www.facebook.com/topciment.microcemento/"/>


     <link rel="canonical" href="https://www.topciment.com/cn/jishucanshu"/>

      <link rel="alternate" href="https://www.topciment.com/cn/jishucanshu" hreflang="zh-CN"/>
     <link rel="alternate" href="https://www.topciment.com/fr/fiches-techniques" hreflang="fr-FR"/>
           <link rel="alternate" href="https://topciment.com/en/data-sheet" hreflang="en-GB"/>
           <link rel="alternate" href="https://topciment.com/pl/karty-techniczne" hreflang="pl-PL" />
           
     <?php include("../assets/includes/root-scripts-head.php"); ?>
 </head>

 <body id="fichas">

     <?php include("includes/root-header.php"); ?>
     <?php include("includes/root-menu-movil.php"); ?>


     <div id="main">
         <div class=" clearfix">
             <!-- Start Fichas técnicas  -->

             <div class="container">
                 <div id="sidebar" class="menuOculto menuMicrocementos stick">
                     <ul>
                         <li>
                             <h1><a href="#top" style="font-size:26px">Descargas</a></h1>
                         </li>
                         <li><a href="#fichas-tecnicas-topciment"> Fichas técnicas</a></li>
                         <li><a href="#catalogo-topciment"> Catálogo</a></li>
                     </ul>
                 </div>
                 <div class="content">
                     <a id="jishucanshu-topciment" class="ancla"></a>
                     <h2 class="en2">技术参数</h2>
                     <p>
                         Descarga nuestro catálogo y nuestras Fichas técnicas para elegir la solución que mejor se ajuste a
                         tus necesidades.
                     </p>
                     <h4 class="card-header mb-3">Sttandard
                     </h4>
                     <p class="links-ft text-dark">
                         <a href="https://www.topciment.com/cn/data-sheets/MicrobaseBI-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microbase L</strong>
                            (双组分微水泥制备)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/MicrobaseBI-XL-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microbase XL</strong>
                         (双组分微水泥制备)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/MicrobaseBI-XXL-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microbase XXL</strong>
                            (双组分微水泥制备)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/MicrodeckBI-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microdeck</strong>
                           (双组分微水泥，用于涂抹地板)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/MicrofinoBI-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microfino</strong>
                            (双组分微水泥，用于涂抹墙面)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/MicrostoneBI-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microstone</strong>
                            (双组分微水泥，用于涂抹室外)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3">Unlimitted</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Unlimitted_Microbase_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Unlimitted Microbase</strong> （双组份微水泥制备）</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Unlimitted_Microdeck_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Unlimitted Microdeck</strong> （双组分微水泥，用于涂抹地板和墙壁）</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3">Evoluttion</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Evoluttion_MicrobaseMN_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Evoluttion演化系列基础</strong> （单组份微水泥制备）</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Evoluttion_MicrofinoMN_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Evoluttion演化系列墙面面层</strong>  (单组份微水泥，用于涂抹墙面)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Evoluttion_MicrodeckMN_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Evoluttion演化系列地面面层</strong> (单组份微水泥，用于涂抹地板)</a><br>
                     </p>

                     <h4 class="card-header mb-3 mt-3">Atlanttic</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/AquacimentBI-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Atlanttic Aquaciment</strong> Atlanttic 大西洋泳池系列</a>
                     </p>
                     <h4 class="card-header mb-3 mt-3">Efectto</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Ficha_Tecnica_Efectto_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Efectto Quartz</strong>（即用型微水泥，涂抹于墙面）</a>
                     </p>
                     <h4 class="card-header mb-3 mt-3">Pure Mettal</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/PureMettal_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Pure Mettal </strong> (（金属涂层和涂料）</a>
                     </p>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Classic Mettal
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Classic_Mettal_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Classic Mettal  </strong>（金属和氧化效果的油漆）。</a>
                         <br>
                        
                     <h5 class="card-header mb-3 mt-3"> Mettal Activator</h5>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Mettal_Activator-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Mettal Activator</strong> (氧化加速剂)</a><br>

                     </p>
                     <h4 class="card-header mb-3 mt-3"> Elitte
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Elitte_Glaze_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Elitte Glaze</strong> （金属效果的釉料）</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Elitte_Glitter_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Elitte Glitter</strong>（珠光效果的釉料）</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Primacem
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Primacem-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem</strong> （具有粘合效果的底漆）</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/PrimacemGRIP-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem Grip</strong> (带骨料的底漆)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/PrimacemABS-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem ABS</strong> （用于吸收性表面的底漆）</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/PrimacemPLUS-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem Plus</strong> （用于非吸收性表面的底漆）</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Impoxy-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Impoxy</strong> (抗潮湿环氧底漆)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Arcocem
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/ArcocemBASIC-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Arcocem Basic</strong> （微水泥的基础颜色）。</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/ArcocemPLUS-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Arcocem Plus</strong>(微水泥单剂颜料)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Topsealer
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/TopsealerPro+Fast_CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer Pro +</strong>
                          (100%固化，防水聚氨酯清漆)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/TopsealerWTonecoat-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer WT One
                                 Coat</strong>
                             (Barniz sellador para microcemento)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/TopsealerWTquickdry-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer WT Quick
                                 Dry</strong> (Barniz sellador para microcemento de secado rápido)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/TopsealerWTantiSlip-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer WT Anti
                                 Slip</strong> (Barniz sellador antideslizante)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/TopsealerDSV-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer DSV</strong> (barniz sellador
                             microcemento base
                             disolvente)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Presealer-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Presealer</strong> (Imprimación
                             tapaporos de piscinas),</a><br>

                     </p>



                     <h4 class="card-header mb-3 mt-3"> Limpieza y mantenimiento</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Ceraciment-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ceraciment</strong> (Cera de
                             mantenimiento para suelos.)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/CeracimentPRO-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ceraciment PRO</strong>
                             (Cera de mantenimiento para suelos de alta resistencia y con acabado metalizado.)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/Ecoclean-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ecoclean</strong> (Limpiador
                             ecológico de uso doméstico.)</a><br>
                         <a href="https://www.topciment.com/cn/data-sheets/EcocleanPRO-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ecoclean PRO</strong>
                             (Limpiador ecológico profesional.)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Herramientas</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/cn/data-sheets/Builtex-CN.pdf" target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Builtex</strong> (Malla de Fibra de
                             vidrio extra plana y flexible)</a><br>

                     </p>

                 </div>
             </div>
             <!-- End Fichas técnicas -->
             <div style="float:none; clear:both; height:50px;"></div>



             <div id="top" class="fullscreen background parallax" style="background-image:url('https://www.topciment.com/imagenes/microcemento-para-suelos.jpg'); " data-img-width="1500" data-img-height="945" data-diff="100">
             </div>


             <!-- Start Catálogo-->

             <div class="container">
                 <div class="content">

                     <a id="catalogo-topciment" class="ancla"></a>
                     <h2>Catálogo Topciment</h2>
                     <p class="texto-catalogo" style="text-align:justify;">Nuestro nuevo catálogo recoge información
                         detallada de
                         todas las
                         soluciones
                         Topciment. Se trata de una herramienta de consulta en la que incorporamos novedades acompañadas de
                         los productos
                         de
                         siempre.
                         Descubre la <strong class="links"><a class="links" href="wenxingshuini/weishuini-gongchang">empresa</a></strong>, la
                         <strong><a class="links" href="wenxingshuini/chanping">forma de
                                 trabajar</a></strong> y los <strong><a class="links" href="wenxingshuini/tuku">项目</a></strong> en los que
                         formamos
                         parte. Gracias a nuestro espíritu innovador centrado en la investigación constante, este catálogo se
                         convierte
                         en algo vivo y en constante renovación. ¡Disfrútalo! .
                     </p>
                     <div class="row  container-fluid ">
                         <div class="col-md-12">
                             <div class="iframe text-center">

                                 <iframe src="https://www.topciment.com/private/#p=1" seamless="seamless" scrolling="no" frameborder="0" allowtransparency="true" allowfullscreen="true" height="900" width="100%">
                                 </iframe>
                             </div>

                         </div>

                     </div>

                 </div>
             </div>
             <!-- End Catálogo -->
         </div>


         <?php include "includes/root-footer.php"; ?>

         <?php include "../assets/includes/root-scripts.php"; ?>
     </div>

     <!-- include jQuery -->

 </body>

 </html>