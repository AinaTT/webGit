 <!DOCTYPE html>
 <html lang="de">

 <head>
     <meta charset="utf-8">
     <meta name="theme-color" content="#000" />
     <meta name="viewport" content="width=device-width" />

     <link rel="icon" type="image/png" href="https://www.topciment.com/favicon.png" />
     <meta name="Language" content="DE" />


     <?php include("../assets/includes/root-styles.php"); ?>

     <link rel="preload" href="../assets/css/fichas-tecnicas.css" as="style" />
     <link rel="stylesheet" href="../assets/css/fichas-tecnicas.css" />

     <title>Technische Datenblätter | Topciment</title>
     <meta name="description"
         content="Laden Sie unseren Katalog und unsere technischen Datenblätter herunter, um die Lösung zu wählen, die Ihren Anforderungen am besten entspricht. " />
     <meta property="og:locale" content="es_ES" />
     <meta property="og:type" content="website" />
     <meta property="og:title" content="Datenblätter | Topciment" />
     <meta property="og:description"
         content="Laden Sie unseren Katalog und unsere Datenblätter herunter und wählen Sie die Lösung, die am besten zu Ihren Anforderungen passt. " />
     <meta property="og:url" content="https://www.topciment.com/de/datenblatter" />
     <meta property="og:site_name" content="Topciment" />
     <meta name="twitter:card" content="summary" />
     <meta name="twitter:description"
         content="Laden Sie unseren Katalog und unsere Datenblätter herunter, um die Lösung zu wählen, die Ihren Anforderungen am besten entspricht. " />
     <meta name="twitter:title" content="Datenblätter | Topciment" />
     <meta name="twitter:site" content="@topciment" />
     <meta name="twitter:creator" content="@topciment" />
     <meta property="article:publisher" content="https://www.facebook.com/topciment.microcemento/" />


     <link rel="canonical" href="https://www.topciment.com/de/datenblatter" />

     <link rel="alternate" href="https://www.topciment.com/de/datenblatter" hreflang="de-DE" />
     <link rel="alternate" href="https://www.topciment.com/fr/fiches-techniques" hreflang="fr-FR" />
     <link rel="alternate" href="https://topciment.com/en/data-sheet" hreflang="en-GB" />
     <link rel="alternate" href="https://topciment.com/pl/karty-techniczne" hreflang="pl-PL" />

     <?php include("../assets/includes/root-scripts-head.php"); ?>
 </head>

 <body id="fichas">

     <?php include("includes/root-header.php"); ?>
     <?php include("includes/root-menu-movil.php"); ?>


     <div id="main">
         <div class=" clearfix">
             <!-- Start Fichas técnicas  -->

             <div class="container">
                 <div id="sidebar" class="menuOculto menuMicrocementos stick">
                     <ul>
                         <li>
                             <h1><a href="#top" style="font-size:26px">Downloaden</a></h1>
                         </li>
                         <li><a href="#fichas-tecnicas-topciment"> Datenblätter</a></li>
                         <li><a href="#catalogo-topciment"> Katalog</a></li>
                     </ul>
                 </div>
                 <div class="content">
                     <a id="fichas-tecnicas-topciment" class="ancla"></a>
                     <h2 class="en2">Technische Daten</h2>
                     <p>Laden Sie unseren Katalog und unsere technischen Daten herunter, um die Lösung zu wählen, die
                         Ihren Anforderungen am besten entspricht. </p>
                     <h4 class="card-header mb-3">Sttandard
                     </h4>
                     <p class="links-ft text-dark">
                         <a href="https://www.topciment.com/de/technische-datenblatter/MicrobaseBI-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microbase
                                 L</strong>
                             (Zweikomponenten-Mikrozement-Zubereitung)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/MicrobaseBI-XL-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microbase
                                 XL</strong>
                             (Zweikomponenten-Mikrozement-Zubereitung)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/MicrobaseBI-XXL-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microbase
                                 XXL</strong>
                             (Zweikomponenten-Mikrozement-Zubereitung)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/MicrodeckBI-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microdeck</strong>
                             (Zweikomponenten-Mikrozement für Fußböden)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/MicrofinoBI-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microfino</strong>
                             (Zweikomponenten-Feinmikrozement)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/MicrostoneBI-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Microstone</strong>
                             (Zweikomponenten-Mikrozement für den Außenbereich)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Aquaciment-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Aquaciment</strong>
                             (Mikrozement für Schwimmbadbeschichtung)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Acricem-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Sttandard Acricem</strong>
                             (Acrylharz für Mikrozementpräparation)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3">Unlimitted</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Unlimitted_MicrobaseBI-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Unlimitted
                                 Microbase</strong> (Zweikomponenten-Mikrozement-Zubereitung)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Unlimitted_MicrodeckBI-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Unlimitted
                                 Microdeck</strong> (Zweikomponenten-Mikrozement für Fußböden)</a><br> </p>
                     <h4 class="card-header mb-3 mt-3">Evoluttion</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Evoluttion_MicrobaseMN_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Evoluttion Microbase</strong>
                             (Monokomponenten-Mikrozement zur Vorbereitung)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Evoluttion_MicrofinoMN_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Evoluttion Microfino</strong>
                             (Monokomponenten-Mikrozement für Wände)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Evoluttion_MicrodeckMN_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Evoluttion Microdeck</strong>
                             (Monokomponenten-Mikrozement für Fußböden)</a><br>
                     </p>

                     <h4 class="card-header mb-3 mt-3">Atlanttic</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/atlanttic-aquaciment-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Atlanttic Aquaciment</strong>
                             (Mikrozement für Schwimmbäder)</a> </p>
                     <h4 class="card-header mb-3 mt-3">Efectto</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Ficha_Tecnica_Efectto_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Efectto Quartz</strong>
                             (Gebrauchsfertiger Mikrozement)</a>
                     </p>
                     <h4 class="card-header mb-3 mt-3">Pure Mettal</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/PureMettal_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Pure Mettal </strong>
                             (Metallischer Mikrozement)</a>
                     </p>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Classic Mettal
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Classic_Mettal_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  ">

                             </i><strong>Classic Mettal </strong> (Metallic-Lackierung)</a> </p>
                     <h5 class="card-header mb-3 mt-3"> Mettal Activator</h5>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Mettal_Activator-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i>
                             <strong>Mettal Activator</strong> (Beschleuniger für rostige Oberflächen)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Elitte
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Elitte_Glaze_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Elitte Glaze</strong>
                             (Metallische Glasuren)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Elitte_Glitter_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Elitte
                                 Glitter</strong>(Metallisch glitzernd)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Primacem
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Primacem-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem</strong>
                             (Haftbrückenprimer mit Klebeeffekt)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/PrimacemGRIP-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem Grip</strong>
                             (Universal-Haftvermittler mit Aggregat)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/PrimacemABS-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem ABS</strong>
                             (Haftvermittler für saugfähige Oberflächen)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/PrimacemPLUS-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Primacem Plus</strong>
                             (Haftvermittler für nicht saugfähige Oberflächen)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Impoxy-DE.pdf" target="_blank"><i
                                 class="fas fa-file-pdf mr-2  "></i><strong>Impoxy</strong> (Zweikomponenten-Epoxidharz
                             - Feuchtigkeitssperre)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Arcocem
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/ArcocemBASIC-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Arcocem Basic</strong>
                             (konzentriertes Pigment zur Herstellung von Dosen)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/ArcocemPLUS-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Arcocem Plus</strong>
                             (Einzeldosis-Pigmente für Mikrozement)</a><br>
                     </p>
                     <h4 class="card-header mb-3 mt-3"> Topsealer
                     </h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/TopsealerPro+Fast_DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer Pro +</strong>
                             (Bikomponenten-Siegellack)</a><br> <a
                             href="https://www.topciment.com/de/technische-datenblatter/TopsealerWTonecoat-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer WT One
                                 Coat</strong>
                             (Versiegelungslack für Mikrozement)</a><br> <a
                             href="https://www.topciment.com/de/technische-datenblatter/TopsealerWTquickdry-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer WT Quick
                                 Dry</strong> (Schnelltrocknender Sealer-Lack für Mikrozement)</a><br> <a
                             href="https://www.topciment.com/de/technische-datenblatter/TopsealerWTantiSlip-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer WT Anti
                                 Slip</strong> (Anti-Rutsch-Versiegelungslack)</a><br> <a
                             href="https://www.topciment.com/de/technische-datenblatter/TopsealerDSV-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Topsealer DSV</strong>
                             (lösemittelhaltiger Mikrozement-Versiegelungslack)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/Presealer-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Presealer</strong>
                             (Schwimmbad-Fibel),</a><br>

                     </p>



                     <h4 class="card-header mb-3 mt-3"> Reinigung und Wartung</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Ceraciment-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ceraciment</strong>
                             (Pflegewachs für Böden)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/CeracimentPRO-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ceraciment PRO</strong>
                             (Pflegewachs für Böden mit hoher Widerstandsfähigkeit und metallischem Finish).</a><br> <a
                             href="https://www.topciment.com/de/technische-datenblatter/Ecoclean-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ecoclean</strong> (Umweltfreundlicher Haushaltsreiniger.)</a><br>
                         <a href="https://www.topciment.com/de/technische-datenblatter/EcocleanPRO-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong> Ecoclean PRO</strong>
                             (Professioneller ökologischer Reiniger.)</a><br> </p>
                     <h4 class="card-header mb-3 mt-3"> Werkzeuge</h4>
                     <p class="text-dark links-ft">
                         <a href="https://www.topciment.com/de/technische-datenblatter/Builtex-DE.pdf"
                             target="_blank"><i class="fas fa-file-pdf mr-2  "></i><strong>Builtex</strong> (Extra flaches und flexibles Glasfasernetz)</a><br>

                     </p>

                 </div>
             </div>
             <!-- End Fichas técnicas -->
             <div style="float:none; clear:both; height:50px;"></div>



             <div id="top" class="fullscreen background parallax"
                 style="background-image:url('https://www.topciment.com/imagenes/microcemento-para-suelos.jpg'); "
                 data-img-width="1500" data-img-height="945" data-diff="100">
             </div>


             <!-- Start Catálogo-->

             <div class="container">
                 <div class="content">

                     <a id="catalogo-topciment" class="ancla"></a>
                     <h2>Catálogo Topciment</h2>
                     <p class="texto-catalogo" style="text-align:justify;">Unser neuer Katalog enthält detaillierte
                         Informationen zu allen Topciment-Lösungen. Es ist ein Referenzwerkzeug, in das wir neben den
                         gewohnten Produkten auch neue Produkte einbauen. Entdecken Sie die
                         <strong class="links"><a class="links"
                                 href="mikrozement/hersteller-mikrozement">firma</a></strong>, die <strong><a
                                 class="links" href="mikrozement/produkte">Arbeitsweise</a></strong> und die
                         <strong><a class="links" href="mikrozement/galerie">Projekte</a></strong>, an denen wir
                         teilnehmen. Dank unseres innovativen Geistes, der auf ständige Forschung ausgerichtet ist, ist
                         dieser Katalog lebendig und in ständiger Erneuerung. Genießen Sie ihn! .
                     </p>
                     <div class="row  container-fluid ">
                         <div class="col-md-12">
                             <div class="iframe text-center">

                                 <iframe src="https://www.topciment.com/private/#p=1" seamless="seamless" scrolling="no"
                                     frameborder="0" allowtransparency="true" allowfullscreen="true" height="900"
                                     width="100%">
                                 </iframe>
                             </div>

                         </div>

                     </div>

                 </div>
             </div>
             <!-- End Catálogo -->
         </div>


         <?php include "includes/root-footer.php"; ?>

         <?php include "../assets/includes/root-scripts.php"; ?>
     </div>

     <!-- include jQuery -->

 </body>

 </html>